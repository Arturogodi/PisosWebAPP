...
def log_message(request):
    ...
    return render(
        request, "hello/log_message.html", {"form": form, "messages": messages}
    )
    # else:
...
def model_idealista(request):
    ...
    return render(
        request,
        "hello/model_idealista.html",
        {"form": form, "propiedad_creada": propiedad_creada},
    )
...
def predict_view(request):
    ...
    if request.method == "POST":
        ...
        # Crear el payload
        payload = {"dataframe_records": [{"features": features}]}
        ...
        if response.status_code == 200:
            ...
            prediction_value = (
                predictions["predictions"][0] if "predictions" in predictions else 0
            )
    ...
    prediction_history = PredictionHistory.objects.all().order_by("-log_date")[
        :5
    ]  # Últimas 5 predicciones
    return render(
        request,
        "hello/predictions.html",
        {
            "bathrooms": bathrooms,
            "isparking": isparking,
            "latitude": latitude,
